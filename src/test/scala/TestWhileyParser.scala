...
class TestWhileyParser extends AnyFunSuite {
  ...
  test("Ident -> ASTIdent - 1") {
    my_assert(WhileyParser.Ident.parseAll("test_Ident"), ASTIdent("test_Ident"))
  }

  test("Ident -> ASTIdent - 2") {
    my_assert_neg(WhileyParser.Ident.parseAll("true"))
  }

  test("Ident -> Error - 1") {
    my_assert_neg(WhileyParser.Ident.parseAll("test_Ident2"))
  }

  test("Ident -> Error - 2") {
    my_assert_neg(WhileyParser.Ident.parseAll("test Ident"))
  }

  test("Literals -> ASTIntLiteral") {
    my_assert(WhileyParser.Literals.parseAll("1337"), ASTIntLiteral(1337))
  }

  test("Literals -> ASTHexLiteral") {
    my_assert(WhileyParser.Literals.parseAll("0xdead69"), ASTHexLiteral(0xdead69))
  }

  test("Literals -> ASTBoolLiteral") {
    my_assert(WhileyParser.Literals.parseAll("true"), ASTBoolLiteral(true))
  }

  test("PackageDecl -> ASTPackageDecl") {
    my_assert(WhileyParser.PackageDecl.parseAll("package lets.test.it"), ASTPackageDecl("lets.test.it"))
  }

  test("ImportDecl -> ASTImportDecl") {
    my_assert(WhileyParser.ImportDecl.parseAll("import lets::test::it"), ASTImportDecl("lets::test::it"))
  }

  test("Type -> ASTType - 1") {
    my_assert(WhileyParser.Type.parseAll("int"), ASTType(ASTPrimitiveType("int"), false))
  }

  test("Type -> ASTType - 2") {
    my_assert(WhileyParser.Type.parseAll("( int)"), ASTType(ASTPrimitiveType("int"), true))
  }

  test("Type -> ASTType - 3") {
    my_assert(WhileyParser.Type.parseAll("(( bool) )"), ASTType(ASTType(ASTPrimitiveType("bool"), true), true))
  }

  test("Expr -> ASTBinaryOp") {
    my_assert(WhileyParser.Expr.parseAll("5 + 5"), ASTBinaryOp(ASTIntLiteral(5), "+", ASTIntLiteral(5)))
  }

  test("Expr -> ASTUnaryOp") {
    my_assert(WhileyParser.Expr.parseAll("-3"), ASTUnaryOp("-", ASTIntLiteral(3)))
  }

  test("Expr -> Mixed Op") {
    my_assert(WhileyParser.Expr.parseAll("a + b * (-4)"), ASTBinaryOp(ASTIdent("a"), "+", ASTBinaryOp(ASTIdent("b"), "*", ASTParanthesis(ASTUnaryOp("-", ASTIntLiteral(4))))))
  }

  test("CodeBlock -> ASTCodeBlock") {
    my_assert(WhileyParser.CodeBlock.parseAll("\n    return 5"), ASTCodeBlock(List((4, ASTReturnStmt(List(ASTIntLiteral(5)))))))
  }
